...
class GoogleCredentialManager:
    def get_creds():
        ...
        try:
            creds = GoogleCredentialManager.__get_creds_logic(original_token_path, token_path, tmp_dir)
        except:
            GoogleCredentialManager.__remove_tmp_token_file(token_path)
            creds = GoogleCredentialManager.__get_creds_logic(original_token_path, token_path, tmp_dir) 
       
        return creds 

    def __get_creds_logic(original_token_path, token_path, tmp_dir):
        ...
        if (os.path.exists(original_token_path) and not os.path.exists(token_path)):
            GoogleCredentialManager.__copy_creds_from_original_token_path(original_token_path, token_path, tmp_dir)
    ...
    def __copy_creds_from_original_token_path(original_token_path, token_path, tmp_dir):
        try:
            makedirs(tmp_dir)
    ...
    def __remove_tmp_token_file(token_path):
        if os.path.exists(token_path):
...
class TimeLoggerCredentialsManager:
    def get_creds():
